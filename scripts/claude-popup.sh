...
#                       throwaway "claude-picker" session, so retarget the popup
...
# Visual identity so it's obvious at a glance which picker is open (pink = Claude).
accent='#d27e99'
border_label=' CLAUDE '
  ...
  emit_list | fzf \
      ...
      --preview '
        ...
        if [ -n "$name" ] && tmux has-session -t "=$name" 2>/dev/null; then
          tmux capture-pane -ep -t "$name"           # live agent: show its screen
        else
...
if [ -z "${dir:-}" ]; then
  ...
  # claude-picker session; hop back to the previous session so that picker
